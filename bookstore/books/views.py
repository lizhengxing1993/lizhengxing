...
def index(request):
	...
	database_hot = Books.objects.get_books_by_type(DATABASE, 3, sort='hot')
...
def detail(request, books_id):
	...
	#用户登录之后，才记录浏览记录
	#每个用户浏览记录对应的redis中的一条信息   格式:'history_用户id':[10,9,2,3,4]
...
def list(request, type_id, page):
	...
	if int(type_id) not in BOOKS_TYPE.keys():
		return redirect(reverse('books:index'))
	# 	根据商品种类id和排序方式查询
	books_li = Books.objects.get_by_type(type_id=type_id, sort=sort)
	# 分页
	...
	# 定义上下文
	type_title = BOOKS_TYPE[int(type)]
	context = {
		'books_li': books_li,
		'books_new': books_new,
		'type_id': type_id,
