...
from order.models import OrderGoodsManager
...
# 	验证用户是否登录
	if not request.session.has_key('islogin'):
		return JsonResponse({'res': 0, 'errms': '用户未登录'})
...
def order_pay(request):
	...
	#登录判断
	if not request.session.has_key('islogin'):
		return JsonResponse({'res': 0, 'ermsg': '用户未登录'})
...
def check_pay(requset):
	'''支付结果'''
	if not requset.session.has_key('islogoin'):
		return JsonResponse({'res': 0, 'errmsg': '用户未登录'})

	passport_id = requset.session.get('passport_id')
	#接收订单id
	order_id = requset.POST.get('prder_id')
